  ...
  return (
    <div style={{minHeight:"100vh",background:"#0f0f14",display:"flex",alignItems:"center",justifyContent:"center",paddingTop:"56px"}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Fredoka+One&family=DM+Sans:wght@300;400;500;600&display=swap'); 
        ...
        .sp-track:hover{background:rgba(30,215,96,0.08)!important;}`}</style>
      
      <div style={{maxWidth:"460px",width:"100%",padding:"0 2rem"}}>
        <div style={{textAlign:"center",marginBottom:"2.5rem"}}>
          <h1 style={{fontFamily:"'Fredoka One', cursive",fontSize:"3rem",color:"#fff",margin:"0 0 0.4rem"}}>now<span style={{color:"#E3000B"}}>.</span></h1>
          <p style={{fontFamily:"'DM Sans', sans-serif",color:"rgba(255,255,255,0.35)",fontSize:"14px",margin:0}}>what's been playing lately</p>
        </div>
        ...
        <div style={{background:"rgba(30,215,96,0.05)",border:"1px solid rgba(30,215,96,0.2)",borderRadius:"20px",padding:"2rem"}}>
          <div style={{display:"flex",alignItems:"center",gap:"10px",marginBottom:"1.5rem"}}>
            ...
            <h2 style={{fontFamily:"'Fredoka One', cursive",fontSize:"1.3rem",color:"#1DB954",margin:0}}>last played</h2>
          </div>

          {loading && <div style={{fontFamily:"'DM Sans', sans-serif",color:"rgba(255,255,255,0.4)",fontSize:"14px",textAlign:"center",padding:"1rem 0"}}>fetching...</div>}

          {track && (
            <a href={track.url} target="_blank" rel="noopener noreferrer" className="sp-track" style={{textDecoration:"none",display:"flex",alignItems:"center",gap:"1rem",padding:"0.75rem",borderRadius:"12px",transition:"background 0.2s"}}>
              {track.img && <img src={track.img} alt="" style={{width:"60px",height:"60px",borderRadius:"8px",flexShrink:0}}/>}
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontFamily:"'DM Sans', sans-serif",fontWeight:600,fontSize:"16px",color:"#fff",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{track.name}</div>
                <div style={{fontFamily:"'DM Sans', sans-serif",fontSize:"13px",color:"rgba(255,255,255,0.5)",marginTop:"2px"}}>{track.artist}</div>
                <div style={{fontFamily:"'DM Sans', sans-serif",fontSize:"12px",color:"#1DB954",marginTop:"4px"}}>♫ {timeAgo(track.playedAt)}</div>
              </div>
            </a>
          )}
            ...
            <div>
              <div style={{display:"flex",gap:"8px",marginBottom:"0.75rem"}}>
                <input 
                  ...
                  style={{flex:1,background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:"8px",padding:"10px 12px",color:"#fff",fontFamily:"'DM Sans', sans-serif",fontSize:"14px",outline:"none"}}
                />
                <button onClick={connect} style={{background:"#1DB954",border:"none",borderRadius:"8px",padding:"10px 16px",color:"#000",fontFamily:"'DM Sans', sans-serif",fontSize:"14px",fontWeight:700,cursor:"pointer"}}>
                  connect
                </button>
              </div>
              {error && <p style={{fontFamily:"'DM Sans', sans-serif",color:"#ff6b6b",fontSize:"12px",margin:"0 0 0.75rem"}}>{error}</p>}
              <button onClick={() => setShowHelp(s => !s)} style={{background:"transparent",border:"none",color:"rgba(255,255,255,0.3)",fontFamily:"'DM Sans', sans-serif",fontSize:"12px",cursor:"pointer",padding:0}}>
