...
const LegoCursor: FC = () => {
  ...
  useEffect(() => {
    ...
    const down = () => { setClicking(true); colorIdx.current=(colorIdx.current+1)%LEGO_COLORS.length; setColor(LEGO_COLORS[colorIdx.current]); };
  ...
  const studs = [[8,4],[20,4],[8,16],[20,16]] as const;
  return (
    <div style={{position:"fixed",top:0,left:0,width:"100%",height:"100%",pointerEvents:"none",zIndex:99999,overflow:"hidden"}}>
      <svg style={{position:"absolute",left:pos.x-14,top:pos.y-14,transform:`scale(${clicking?0.85:1})`,transition:"transform 0.1s",filter:"drop-shadow(1px 2px 3px rgba(0,0,0,0.4))"}} width="28" height="28" viewBox="0 0 28 28">
